...
import { readFiles } from './fixtures/directus-sdk/read-files.data'
import { mockDirectusUpload, requestMock, state, uploadFilesMock } from './fixtures/directus-sdk/upload-files.mock'
import { getDirectusFileUrl, uploadDirectusFile, uploadDirectusFiles } from '../src/runtime/composables/files'
...
vi.mock('../src/runtime/composables/directus', async () => {
  const { requestMock } = await import('./fixtures/directus-sdk/upload-files.mock')
  return {
...
vi.mock('@directus/sdk', async () => {
  const { uploadFilesMock } = await import('./fixtures/directus-sdk/upload-files.mock')
  return {
...
beforeEach(() => {
  ...
  state.capturedFormData = null
})

describe('getDirectusFileUrl', () => {
  it('builds a URL from a string file ID', () => {
    expect(getDirectusFileUrl(FILE_1.id)).toContain(`assets/${FILE_1.id}`)
  })

  it('accepts a DirectusFile object', () => {
    const file: DirectusFile = { id: FILE_1.id }
    expect(getDirectusFileUrl(file)).toContain(`assets/${FILE_1.id}`)
  })

  it('produces a clean URL with no query string when no options are passed', () => {
    expect(getDirectusFileUrl(FILE_1.id)).not.toContain('?')
  })

  it('appends width and height matching the fixture image dimensions', () => {
...
describe('uploadDirectusFiles', () => {
  it('appends each file under the "file" key so multiple files are not overwritten', async () => {
    mockDirectusUpload().withVersion('latest')
    ...
    const result = await uploadDirectusFiles([{ file: file1 }, { file: file2 }])

    expect(state.capturedFormData!.getAll('file')).toHaveLength(2)
    expect(state.capturedFormData!.getAll('file')[0]).toBe(file1)
    expect(state.capturedFormData!.getAll('file')[1]).toBe(file2)
    expect(result).toStrictEqual(readFiles.admin)
  })
  ...
  it('includes metadata fields alongside the file', async () => {
    ...
    const result = await uploadDirectusFiles([{ file, data: { title: FILE_1.title } }])

    expect(state.capturedFormData!.get('title')).toBe(FILE_1.title)
    expect(state.capturedFormData!.getAll('file')).toHaveLength(1)
    expect(result).toStrictEqual(readFiles.admin)
  })
...
describe('uploadDirectusFile', () => {
  ...
  it('returns the result directly when it is not an array', async () => {
    mockDirectusUpload().withVersion('latest')
    requestMock.mockResolvedValue(FILE_2)
  ...
  it('delegates to uploadDirectusFiles with a single-item array', async () => {
    ...
    expect(state.capturedFormData!.getAll('file')).toHaveLength(1)
    expect(state.capturedFormData!.getAll('file')[0]).toBe(file)
  })
})
